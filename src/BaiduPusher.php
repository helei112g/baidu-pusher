 ...
 *         
 */
class BaiduPusher extends BaseSDK
{
    const VERSION = '3.0.0';
     ...
     */
    public function __construct($apiKey = null, $secretKey = null, $curlopts = array())
    {
        
        if ($apiKey !== null && $secretKey !== null) {
            $this->setApiKey($apiKey);
            $this->setSecretKey($secretKey);
        } elseif ($apiKey !== null || $secretKey !== null) {
            $loss = $apiKey == null ? 'apikey' : 'secretKey';
            throw new \Exception("need $loss.");
        } else {
            $this->setApiKey(config('pusher.default_apiKey'));
            $this->setSecretKey(config('pusher.default_secretkey'));
        }
     ...
     * @return mixed
     */
    public function __call($name, $originArgs)
    {
        $className = __CLASS__;
    ...
    protected function verifyGroup(&$payload)
    {
        ...
        elseif (array_key_exists('selector', $payload)) {             
